...
#
def create_generation_volumes(volume,skeletonize_method=None, graph_creation="networkx", threshold=-2000, min_branch_children=20,remove_trachea=True, distance_error_threshold=5, verbosity=0):
    ...
    binarised_volume = binary_fill_holes((np.abs(skeleton_dt-skeleton_MST_dt)<distance_error_threshold) & binarised_volume)
    ...
    if skimage.measure.label(skeleton_generation>0).max()>1:
        print("the final volume is disconnected!")
